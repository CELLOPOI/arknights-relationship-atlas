...
const {graph,evidence}=applyIdentityCorrections(read(path.join(directory,'graph.json')),read(path.join(directory,'evidence.json')));
const replacements=new Map(identityCorrections.corrections.filter(c=>c.action==='replace_evidence').map(c=>[c.relationshipId,c.replacement]));
...
assert.equal(nodeMap.size,396);assert.equal(graph.edges.length,3689);assert.equal(new Set(graph.edges.map(e=>e.id)).size,3689);
assert.equal(graph.edges.filter(e=>e.kind==='mutual').length,2788);assert.equal(graph.edges.filter(e=>e.kind==='awareness').length,901);
...
for(const e of graph.edges){
  ...
  assert.deepEqual(Object.keys(evidence[e.id]).sort(),['note','quote','sources']);
  if(replacements.has(e.id)){assert.deepEqual(evidence[e.id],replacements.get(e.id));continue;}
  assert.equal(evidence[e.id].quote,r.关键原文);assert.equal(evidence[e.id].note,r.备注);
...
assert.ok(!graph.factions.some(f=>f.id==='lungmen'));assert.equal(Object.keys(evidence).length,3689);
console.log('PASS: 396 nodes, 3689 corrected edges, 901 directed awareness records; scoped corrections, remaining text and provenance, stable IDs and direction regressions verified.');
