...
「泰拉群像」新增喜好功能时，继续以用户指定的本地 `arknights-rebuild` 工程作为前端参考资料。开始界面实现前，先读该工程的 `README.md`、`src/README.md` 与 `docs/SOURCE_LEARNING.md`，再按下面的模块入口核对源码；已有桌面、手机截图可以辅助理解构图，但要检查是否对应当前源码。
