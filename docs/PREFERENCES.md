...
每周最多 300 道、连续 28 天最多 1,200 道、每人物 28 天最多 6 次展示；新题按无序形态对防重复 84 天，历史无形态任务按原人物对约束。每次新派题按 60% 普通随机、30% 近分对位、10% 补覆盖分流。普通随机先选人物再选形态；近分对位面向全榜的合格形态/NPC，匹配分差不超过 5 分且名次范围重叠的不同人物，缺少合适近邻时回退补覆盖；补覆盖直接选择缺样本形态，再随机选其他人物的可用形态。所有形态仍共享人物曝光额度。派发预留，跳过和自然过期占额；名录发布或明确资源故障作废待答并退额。休息间隔 `PREFERENCE_REST_INTERVAL=50` 与统计 `PREFERENCE_WEIGHT_TOTAL_CAP=50` 独立配置。
...
近分策略为 `proximity-form-v1`，优先新增参与来源并按近分曝光轮转，给直接判断较少的合法组合补题；依据写入 `proximity_task_issued`。派题复用现有快照，不重新拟合模型；候选单次归组，再针对目标查找近分对象，历史计数仍实时从数据库聚合。比例与阈值是试运行规则，仍需通过后续数据观察区间、全池覆盖和纯随机策略对照。配置、回退及统计解释见[全榜近分对位](PREFERENCES_STATISTICS.md#全榜近分对位2026-09-27)。
