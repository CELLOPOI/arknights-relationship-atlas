...
import { transitionSection, type SectionDirection } from './section-navigation';
import './styles/graph.css';
import './styles/site.css';
import './styles/community.css';
import './styles/sources.css';

const entryFor = (path: string) => /^\/sources\/?$/.test(path) ? 'sources' : /^\/game\/?$/.test(path) ? 'game' : 'atlas';
...
document.addEventListener('atlas:section-navigation', event => {
  const { href, direction } = (event as CustomEvent<{ href: string; direction: SectionDirection }>).detail;
  event.preventDefault();
  void navigate(new URL(href, location.href), direction);
});
document.addEventListener('click', event => {
  ...
  if (url.origin !== location.origin || !['/', '/game', '/game/'].includes(url.pathname) || activeEntry === 'sources') return;
  if (entryFor(url.pathname) === activeEntry) { cancelPendingNavigation(); return; }
  event.preventDefault();
  void navigate(url, entryFor(url.pathname) === 'game' ? 1 : -1);
});
window.addEventListener('popstate', event => {
  if (entryFor(location.pathname) === activeEntry) { cancelPendingNavigation(); return; }
  // 旧图谱不能把游戏历史当成首页路由处理。
  event.stopImmediatePropagation();
  void navigate(new URL(location.href), entryFor(location.pathname) === 'game' ? 1 : -1, true);
});
