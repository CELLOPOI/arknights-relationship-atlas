...
const draft = preferenceDraft(`characters-v3:${props.state.choice_order_seed}`, {
  ...
  supportVersion: 0, dirty: false, practice: false, practiceTask: null as Task | null, pause: false, answered: 0,
...
try { localRecords.value = JSON.parse(localStorage.getItem('terra-preference-practice') || '[]').filter((x: RecordItem) => x && typeof x.left_id === 'string').slice(0, 500); } catch { localRecords.value = []; }
...
function newPractice() {
  ...
  const pick = [...pool.value];
  const first = pick.splice(Math.floor(Math.random() * pick.length), 1)[0]!;
  const second = pick[Math.floor(Math.random() * pick.length)]!;
...
async function answer(outcome: string, winnerId?: string) {
  ...
  if (practice.value) {
    ...
    try { localStorage.setItem('terra-preference-practice', JSON.stringify(localRecords.value)); } catch { message.value = '本机储存不可用，练习记录仅保留在当前页面。'; }
    newPractice(); return;
  }
