...
const { selectedKind, selectedScope, windowDays, search, historyId } = toRefs(preferenceDraft(`results-v3:${props.kind || 'people'}:${props.objectId || ''}`, { selectedKind: 'random', selectedScope: 'form', windowDays: 84, search: '', historyId: '' }));
...
const computationFailed = computed(() => snapshot.value?.payload.status === 'solver_failed'
  || snapshot.value?.payload.rows.some(row => row.status === 'solver_failed'));
const hasRankIntervals = computed(() => snapshot.value?.payload.rows.some(row => row.rank_interval));
...
function statusText(status?: string) {
  return ({ disconnected: '还缺一些对比', unstable: '结果未稳', solver_failed: '统计未完成', uncertain: '排名未稳', sensitive: '计分影响较大', separated: '还需更多对比', ready: '可供参考', support_insufficient: '支持人数还少', reference_insufficient: '可排名人数还少' } as Record<string, string>)[status || ''] || '还需更多投票';
}
async function load() {
  ...
  <section class="preference-results">
    ...
    <div class="pref-row">
      <div v-if="!props.kind" class="pref-tabs" aria-label="统计口径">
        <button v-for="item in [['random','随机好感'],['support','厨力支持'],['composite','综合榜']]" :key="item[0]" :aria-pressed="kind === item[0]" @click="selectedKind = item[0]!">{{ item[1] }}</button>
      </div>
    ...
    <template v-if="snapshot && !loading">
      ...
      <p v-if="kind === 'random' && computationFailed" class="pref-message" role="status">统计未完成，已有选择已保留。</p>
      ...
      <details v-if="['composite','random'].includes(kind)" class="pref-method">
        ...
        <p v-if="kind === 'random' && computationFailed">本次计算未完成，暂不提供分数区间与名次。</p>
        <p v-if="kind === 'random' && hasRankIntervals">范围仅比较本次可排名的人物。相邻名次可能互换。</p>
      ...
      <div class="pref-table-scroll"><table class="pref-table"><thead><tr>
        ...
        <template v-else-if="kind === 'random'"><td>{{ row.rank ? `第 ${row.rank} 位` : statusText(row.status) }}<small v-if="row.rank_interval" class="pref-rank-interval">可能第 {{ row.rank_interval[0] }}–{{ row.rank_interval[1] }} 位</small></td><td>{{ number(row.score) }}<span v-if="row.interval?.[0] != null && row.interval?.[1] != null">（{{ number(row.interval[0]) }}–{{ number(row.interval[1]) }}）</span></td><td>{{ row.raw_comparisons ?? row.comparisons }} / {{ row.comparisons }}</td><td>{{ number(row.weighted_evidence) }}</td><td>{{ row.participants }} / {{ number(row.effective_participants) }} / {{ row.opponents }}</td></template>
