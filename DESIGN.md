...
站点已确定更名为「方舟关系与喜好」，命名与默认浏览器标题以 [PRODUCT.md](PRODUCT.md#brand-commitments) 为准，前端已同步。新增「喜好」分区已实现：共用次级页面，左侧「人物 / 皮肤」切换，主体分别承接双人物比较与干员外观网格；右侧职业控制只在皮肤视图出现。移动端保留左侧紧凑切换区，职业按钮放在主体工具区右侧，避免两侧控制同时挤占网格。该约定扩展现有黑白与青色体系，细节见[喜好开发需求](docs/POPULARITY_REQUIREMENTS.md)。
...
响应条件来自实际媒体查询：1200px 调整图谱密度；1100px 调整门户比例并将顶部导航收进菜单，给站点名称和反馈入口留出空间；760px 顶栏高 68px，阵营上下布局，目录从高度 51% 开始，图谱侧栏改为横向阵营栏；380px 以下站名使用 16px 紧凑行高并隐藏英文辅标。横屏且高度不超过 560px 时，顶栏高 54px，阵营粒子占上方 56%，文字从 56% 开始；图谱隐藏头像条，单阵营总览用横向网格并减少 fit 留白。这些是当前实现条件，不承诺覆盖所有设备比例。
...
皮肤 V2 使用真实半身立绘融入暗底，保留脸与肩胸；默认灰阶，hover / focus-visible 彩色与克制缩放，触屏首点打开详情，减少动画关闭变换。八职业与手机两列不变。半身焦点和复核存于名录外的维护清单，详情保留完整构图。滚轮/触控翻页顺序为首页→关系→游戏→喜好，到喜好底部停止。来源与版权页通过链接进入并正常滚动，不参与手势翻页；普通点击与历史按页面顺序保留转场方向；弹窗、输入、长列表和图谱缩放保留原交互。
