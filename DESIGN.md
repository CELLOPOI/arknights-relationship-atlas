...
站点已确定更名为「泰拉群像」，命名与默认浏览器标题以 [PRODUCT.md](PRODUCT.md#brand-commitments) 为准，前端已同步。新增「喜好」分区已实现：共用次级页面，左侧「人物 / 皮肤」切换，主体分别承接双人物比较与干员外观网格；右侧职业控制只在皮肤视图出现。移动端保留左侧紧凑切换区，职业按钮放在主体工具区右侧，避免两侧控制同时挤占网格。该约定扩展现有黑白与青色体系，细节见[喜好开发需求](docs/POPULARITY_REQUIREMENTS.md)。
